...
import {
    ...
    getUserByNicUri,
    getUsersUri,
    getUserSuspensionByNicUri, lockSubmissionUri,
    notesFromReviewUri,
    reviewSubmissionUri,
    suspendUserUri, unlockSubmissionUri,
...
import {SubmissionLockRequestModel} from "./models/SubmissionLockRequestModel";
...
export namespace DoctorServices {
    ...
    export async function lockSubmission(submissionId: number, doctorId: number): Promise<void> {
        const body = {doctorId: doctorId} as SubmissionLockRequestModel;
        await post(lockSubmissionUri(submissionId), JSON.stringify(body));
    }
